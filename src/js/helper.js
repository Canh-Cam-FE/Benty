...
export function countUpInit() {
	...
	countUpElements.forEach((element) => {
		let raw = element.getAttribute("data-number").trim();

		// Lấy hậu tố (như +, %, M...)
		let suffix = raw.replace(/[\d.,]/g, "");
		let numberMatch = raw.match(/[\d]+([.,]\d+)?/);
		if (!numberMatch) return;

		let numberStr = numberMatch[0];
		let decimalPart =
			numberStr.includes(",") || numberStr.includes(".") ? numberStr.split(/[.,]/)[1] : "";
		let leadingZeros = numberStr.length - parseFloat(numberStr).toString().length;

		let target = parseFloat(numberStr.replace(",", "."));
		let decimalPlaces = decimalPart.length;

		element.innerHTML = `<span class="count-value">0</span>${suffix}`;

		const countUp = new CountUp(element.querySelector(".count-value"), target, {
			duration: 3,
			decimal: ",",
			...
			formattingFn: (n) => {
				let base = n.toFixed(decimalPlaces);
				let str = base.padStart(base.length + leadingZeros, "0");
				return str;
			},
		});
